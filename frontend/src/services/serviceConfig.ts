...
async function delet(url: string, params?: Record<string, any>): Promise<any> {
  return instance
    .delete(url, { params })
    .then((response: AxiosResponse) => response.data)
    .catch((error: AxiosError) => requestFailureCallback(url, error));
...
async function formDataPut(url: string, data?: FormData): Promise<any> {
...
export { get, post, delet, formDataPut, formDataPost };
